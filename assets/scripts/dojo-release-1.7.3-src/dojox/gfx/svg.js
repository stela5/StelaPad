...
// !! BUG FIX !! Remove defs reference when related object is removed (see http://bugs.dojotoolkit.org/ticket/14037)
var defsLookup = {};
...
// !! BUG FIX !! Remove defs reference when related object is removed (see http://bugs.dojotoolkit.org/ticket/14037)
if (this.parent.parent.parent.getUID) {
	// fill
	var f = "url(#" + fill.getAttribute("id") +")";
	// level 1
	if (this.parent.getUID() in defsLookup) defsLookup[this.parent.getUID()].push(f);
	else defsLookup[this.parent.getUID()] = [f];
	// level 2
	if (this.parent.parent.getUID() in defsLookup) defsLookup[this.parent.parent.getUID()].push(f);
	else defsLookup[this.parent.parent.getUID()] = [f];
	// level 3
	if (this.parent.parent.parent.getUID() in defsLookup) defsLookup[this.parent.parent.parent.getUID()].push(f);
	else defsLookup[this.parent.parent.parent.getUID()] = [f];
	// check if level 4 exists
	if (this.parent.parent.parent.parent.getUID) {
		// level 4
		if (this.parent.parent.parent.parent.getUID() in defsLookup) defsLookup[this.parent.parent.parent.parent.getUID()].push(f);
		else defsLookup[this.parent.parent.parent.parent.getUID()] = [f];
...
// !! BUG FIX !! Remove defs reference when related object is removed (see http://bugs.dojotoolkit.org/ticket/14037)
var defs = this._getParentSurface().defNode;
var ref = svg.getRef(shape.rawNode.getAttribute("fill"));
if (ref) {
	defs.removeChild(ref);
} else {
	var groupId = shape.getUID();
	if (groupId in defsLookup) {
		var fillArr = defsLookup[groupId];
		for(var i=0; i<fillArr.length; i++){
			ref = svg.getRef(fillArr[i]);
			if (ref) defs.removeChild(ref);
		}
		delete defsLookup[groupId];
	}
}
